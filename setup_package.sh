...
if [ -d "${git_dir}/calibGenACD-master" ]; then
    ...
    ##(DO not use this as older paths can cause an error here!: git clone "https://github.com/fermi-lat/calibGenACD.git" calibGenACD-master 
    ## Use the calibgenacd version in adesai90 github.
fi

cd ..
MY_DIR=$(pwd)
echo "Software will install at:"
echo "${MY_DIR}"


echo "Currently there is only one option for the setup of ACD calibrations software."
echo "Setup using links to /sdf/group/fermi/"
...
echo "......................................................................................................................."
echo "Note: A future additional setup option 2 using a container which links to the old /afs paths by using bind mount is in development in one of the github branches, but due to lower priority, development is paused."
echo "......................................................................................................................."

echo "Running Setup using links to /sdf/group/fermi/"
export CONDA_PREFIX=/sdf/home/a/abhishek/miniconda
export PATH=${CONDA_PREFIX}/bin/:$PATH
source ${CONDA_PREFIX}/etc/profile.d/conda.sh
conda activate acd_env
source ${git_dir}/op1_config.sh
source ${git_dir}/op1_setup.sh
chmod +x ${git_dir}/gcc_linker
chmod +x ${git_dir}/gpp_linker
...
if [ "$answer" = "yes" ]; then
    ...
    --with-cc=${git_dir}/gcc_linker \
    --with-cxx=${git_dir}/gpp_linker --debug=explain $* > build_out.log 2> build_err.log
fi
...
#    --exclude=workdir --supersede=${RELEASE} --rm --compile-opt \
#    --with-cc=${git_dir}/gcc_linker \
#    --with-cxx=${git_dir}/gpp_linker --debug=explain $* > build_ou2.log 2> build_err2.log
