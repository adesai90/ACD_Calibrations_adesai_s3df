...
Bool_t AcdCalibLoop_Digi::attachChains() {
  ...
  if (digiChain != 0) {
    m_digiEvent = 0;
    // ---AD changed: Try multiple branch names for DigiEvent
    TBranch* branch = digiChain->FindBranch("DigiEvent"); // ---AD changed
    if (branch == 0) { branch = digiChain->FindBranch("Digi"); } // ---AD changed
    if (branch == 0) { // ---AD changed
      cerr << "ERR: Could not find DigiEvent or Digi branch" << endl; // ---AD changed
      return kFALSE; // ---AD changed
    } // ---AD changed
    cout << "Found branch: " << branch->GetName() << endl; // ---AD changed
    digiChain->SetBranchAddress(branch->GetName(), &m_digiEvent); // ---AD changed
