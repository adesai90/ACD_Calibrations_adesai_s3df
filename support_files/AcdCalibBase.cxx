...
//  outfile.open("out.txt");
  ...
  // ---AD changed: Setup DigiEvent aliases before reading events
  if ( m_chains[AcdCalib::DIGI] != 0 ) { setupDigiEventAliases(m_chains[AcdCalib::DIGI]); } // ---AD changed
...
// ---AD changed: Add helper function for DigiEvent alias setup
void AcdCalibBase::setupDigiEventAliases(TChain* chain) { // ---AD changed
  if (chain == 0) return; // ---AD changed
  TTree* tree = chain->GetTree(); // ---AD changed
  if (tree == 0) return; // ---AD changed
  if (tree->FindBranch("DigiEvent") == 0) { // ---AD changed
    cout << "Warning: DigiEvent branch not found, creating aliases" << endl; // ---AD changed
    chain->SetAlias("acd_data", "DigiEvent"); // ---AD changed
  } // ---AD changed
} // ---AD changed
